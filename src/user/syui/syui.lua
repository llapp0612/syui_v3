...
function M.TargetsTarget()
    local slot = getTargetSlot()
    if(slot == nil or slot == '')then return end
    local tt_slot = getDbProp('SERVER:Entities:E'..getTargetSlot()..':P7')
	local ui = 'ui:interface'
	local target = getUI(ui..':target')
	local tt = getUI(ui..':tartar')
	local ply = getUI(ui..':player')
	local lnk = tt_slot
	
	--if(tt_slot < 255 or tt_slot ~= nil)then
	--    lnk = tt_slot
	--end
	--if(target.title == '' or isTargetNPC() or getTargetLevel() <= 0 or tt_slot == nil)then
	--	lnk = 255
	--end
	--if(ply.title == target.title)then
	--    lnk = 0
	--end
	runAH(nil,'set_tartar_name','slot=' .. tostring(lnk) .. '|target=ui:interface:tartar:uc_title')
end

function M.tt_to_target()
    local slot = getTargetSlot()
    if(slot == nil or slot == '')then return end
	local ui = getUI('ui:interface')
	local ply = ui:find("player")
	local tt = ui:find("tartar")
	local cm
	if(ply.title == tt.title)then
	    cm = 'self_target'
	else
	    cm = 'assist_target'
	end
...
function M.tt_bar()
    local slot = getTargetSlot()
    if(slot == nil or slot == '')then return end
	local jvalue = getDbProp("UI:VARIABLES:BARS:TARTAR:HP") / 1.27
	if(jvalue < 0)then
	    jvalue = 0
	end
	setDbProp("UI:VARIABLES:BARS:TARTAR:HP_PERCENT", math.floor(jvalue))
	tt_window_open()
	M.update_tt_bar()
end

function M.update_tt_bar()
    local tt = getUI('ui:interface:tartar')
    if(tt.title == '')then
       setDbProp("UI:VARIABLES:BARS:TARTAR:HP_PERCENT", 0)
    end
end

function tt_window_open()
    local tt = getUI('ui:interface:tartar')
	if(not tt.active)then
        runAH(nil, 'show_hide', 'tartar')
	end
end
