...
class CActionHandlerSetTartarName : public IActionHandler
{
	virtual void execute(CCtrlBase *pCaller, const string &Params)
	{
		...
		findInterfacePath(sTitleTartar, pCaller);
		...
		if (CInterfaceExpr::eval(sSlot, evValue, NULL))
		{
			...
			if (nSlot > -1)
			{
				...
				/*if (nDBid != 0)
				{
					...
					pIM->addServerID(sTitleTartar, nDBid, pSPPRN);
				}
				else
				{*/
					if (pE != NULL)
					{
						TartarName = pE->getDisplayName();
						TartarTitle = pE->getTitle();
						if (TartarName.empty() && !TartarTitle.empty())
						{
							TartarName = TartarTitle;
						}
						if(TartarName.empty() && TartarTitle.empty())
						{
							//TartarName = "";
			...
			evUCStr.setUCString(TartarTitle);
			CInterfaceLink::setTargetProperty(sTitleTartar, evUCStr);
		}
	}
};
